    ...
    async execute (bot, packet) {
        ...
        if (bot.isLoggingIn) {
            return await bot.send("v", {
                n: bot.envData.username,
                a: bot.envData.apiKey,
            });
        }
        ...
        // Disabled powers
        if (bot.envData.disabledPowers) {
            for (const k of bot.envData.disabledPowers) {
        ...
        j2Packet.n = `${bot.settings.nick}##${bot.settings.status}`;
        ...
        j2Packet.W = Buffer.from(
            JSON.stringify({
                Pcplus: bot.settings.pcplus,
                avatarSettings: Buffer.from(
                    JSON.stringify({
                        avatareffect: bot.settings.avatareffect,
                        avatarframe: bot.settings.avatarframe,
                        avatarspeed: bot.settings.avatarspeed,
                        avatarcolor: bot.settings.avatarcolor,
                    })
                ).toString("base64"),
                Stealth: bot.settings.stealth,
